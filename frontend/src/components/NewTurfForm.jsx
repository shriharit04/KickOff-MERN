...
function NewTurfForm() {
    ...
    async function addNewTurf(ev) {
        ev.preventDefault();
        try {
            await axios.post('/lister/turf/new', { name, address, desc, addedPhotos, price, open, close, size });
            setShowPopup(true)
        } catch (error) {
                ...
                <div className="grid grid-cols-3  gap-2lg:grid-cols-6 md:grid-cols-4">
                    {addedPhotos.length > 0 && addedPhotos.map(link => (
                        <div className='h-32 flex' key={link}>
                            <img className='rounded-2xl w-full object-cover' src={`${import.meta.env.VITE_BACKEND_URL}uploads/${link}`} alt={link} />
                        </div>
                    ))}
